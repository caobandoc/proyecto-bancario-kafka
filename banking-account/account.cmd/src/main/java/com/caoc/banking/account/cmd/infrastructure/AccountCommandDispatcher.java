...
public class AccountCommandDispatcher implements CommandDispatcher {
    ...
    public void send(BaseCommand command) {
        ...
        if (handlers == null || handlers.isEmpty()) {
            throw new IllegalArgumentException("El command handler no fue registrado");
        }

        if (handlers.size() > 1) {
            throw new IllegalArgumentException("No puede enviar un command que tiene mas de un handler");
        }
